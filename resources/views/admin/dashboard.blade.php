<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded shadow-sm my-9 p-4 flex gap-1">
                <button onclick="openDialog()" class="px-4 py-2 bg-teal-500 hover:bg-teal-700 text-white transition-all duration-150 rounded shadow">Add Radio channel</button>
                <!-- <a href="{{ url('admin/tvs') }}" class="px-4 py-2 bg-orange-500 hover:bg-orange-700 text-white transition-all duration-150 rounded shadow text-center">Manage Tv channels</a> -->
            </div>
            @if(count($channels) == 0)
                <div class="bg-white rounded shadow-sm p-2">
                    <h2 class="text-xl text-teal-500">No channel yet!</h2>
                </div>
            @endif
            @foreach($channels as $item)
            <div class="grid grid-cols-12 mb-1 gap-1 p-2 md:gap-4 bg-blue-900/50 rounded border border-blue-900 shadow hover:shadow-xl hover:border hover:border-blue-600 text-white hover:bg-gray-900/50 transtion-all duration-150">
                    ...
                    <x-drop>
                        <ul class="text-center">
                            <li class="mb-1 border-b">
                                <a href="{{ url('play') }}/{{ $item->id }}" class="p-1 bg-teal-500 hover:bg-teal-700 text-white transition-all duration-150 block">VIEW</a>
                            </li>
                            <li class="mb-1 border-b">
                                <a href="{{ url('admin/editradio') }}/{{ $item->id }}" class="p-1 bg-orange-500 hover:bg-orange-700 text-white transition-all duration-150 block">EDIT</a>
                            </li>
                            <li class="mb-1 border-b">
                                <a href="{{ url('admin/deleteradio') }}/{{ $item->id }}" class="p-1 bg-red-500 hover:bg-red-700 text-white transition-all duration-150 block">DELETE</a>
                            </li>
                        </ul>
                    </x-drop>
                </div>
    ...
    <div id="dialog" class="fixed p-2 pt-9 bg-gray-600/70 w-full h-full top-0 overflow-auto hidden">
        <form action="{{ url('admin/storeradio') }}" method="post" class="bg-white rounded p-4 max-w-4xl mx-auto " enctype="multipart/form-data">
            <h2 class="text-2xl text-teal-500 mb-4">Add a new Radio channel</h2>
